...
def prepare_data_for_timelag_models(
        full_dtw_seg_files: list, sinsy_full_round_seg_files: list, timelag_dir):
    """
    ...
    # 手動設定したフルラベルファイルを複製
    print('Copying full_dtw_seg files')
    for path_lab in tqdm(full_dtw_seg_files):
        copy(path_lab, f'{label_phone_align_dir}/{basename(path_lab)}')

    # Sinsyで生成したフルラベルファイルを複製
    print('Copying sinsy_full_round_seg files')
    for path_lab in tqdm(sinsy_full_round_seg_files):
        copy(path_lab, f'{label_phone_score_dir}/{basename(path_lab)}')


def prepare_data_for_duration_models(full_dtw_seg_files: list, duration_dir):
    """
...
def prepare_data_for_acoustic_models(
        full_dtw_seg_files: list, sinsy_full_round_seg_files: list, wav_files: list, acoustic_dir):
    """
    ...
    for i, path_wav in enumerate(wav_files, 1):
        ...
        segment_wav(path_wav, wav_dir, corresponding_full_dtw_seg_files)
    ...
    # Sinsyで生成したフルラベルファイルを複製
    print('Copying sinsy_full_round_seg files')
    for path_lab_in in tqdm(sinsy_full_round_seg_files):
...
def main(path_config_yaml):
    ...
    db_root = expanduser(config['db_root'])

    full_dtw_seg_files = sorted(glob(f'{out_dir}/full_dtw_seg/*.lab'))
    sinsy_full_round_seg_files = sorted(glob(f'{out_dir}/sinsy_full_round_seg/*.lab'))
